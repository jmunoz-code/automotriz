...
class Clase1(APIView):
    def get(self, request):
        data = TipoCombustible.objects.order_by('-id').all()
        datos_json = TipoCombustibleSerializer(data, many=True)
    ...
    def post(self, request):
        ...
        serializer = TipoCombustibleSerializer(data=modified_data)  #instantiate the serializer
        try:
...
class Clase2(APIView):
    ...
    def get(self, request, id):
        try:
            tipo = TipoCombustible.objects.filter(id=id).get()
           
            serializer = TipoCombustibleSerializer(tipo)
    ...
    def put(self, request, id):
        ...
        serializer = TipoCombustibleSerializer(data=modified_data) 
        try:
    ...
    def delete(self, request, id):
        try:
            tipo = TipoCombustible.objects.get(id=id)
            tipo.delete()
            return Response(status=HTTPStatus.NO_CONTENT)  # Código 204 No Content
        except tipo.DoesNotExist:
            raise NotFound(f"No se encontró forma de tipo Combustible: {id}")
        except Exception as e:
