...
from tipoVehiculo.serializers import TipoVehiculoSerializer
from .models import TipoVehiculo


class Clase1(APIView):
    def get(self, request):
        data = TipoVehiculo.objects.order_by('-id').all()
        datos_json = TipoVehiculoSerializer(data, many=True)
    ...
    def post(self, request):
        ...
        serializer = TipoVehiculoSerializer(data= modified_data)  #instantiate the serializer
        try:
...
class Clase2(APIView):
    ...
    def get(self, request, id):
        try:
            tipo = TipoVehiculo.objects.filter(id=id).get()
    ...
    def put(self, request, id):
        ...
        try:
            tipo = TipoVehiculo.objects.filter(id=id).get()
            serializer =  TipoVehiculoSerializer(tipo, data=modified_data, partial=True)
    ...
    def delete(self, request, id):
        try:
            tipo = TipoVehiculo.objects.get(id=id)
            tipo.delete()
