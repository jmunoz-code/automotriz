...
class Clase1(APIView):
    def get(self, request):
        data = Marca.objects.order_by('-id').all()
        datos_json = MarcaSerializer(data, many=True)
    ...
    def post(self, request):
        ...
        serializer = MarcaSerializer(data=modified_data)  #instantiate the serializer
        try:
...
class Clase2(APIView):
    def get(self, request, id):
        try:
            data = Marca.objects.filter(id=id).get()
    ...
    def put(self, request, id):
        ...
        try:
            marca = Marca.objects.filter(id=id).get()
            serializer =  MarcaSerializer(marca, data=modified_data, partial=True)
