...
export default {
  ...
  setup() {
    ...
    const handleSubmit = () => {
      ...
      if (!formData.value.tipo_marca.trim()) {
        mostrarMensaje('Por favor, ingrese Tipo Marca.', 'error');
        return;
      }
    ...
    const obtenerTipo = async (id) => {
      try {
        const apiUrl = `${import.meta.env.VITE_API_URL}marca/${id}/`;
        console.log(apiUrl);
        ...
        if (!respuesta.ok) {
          console.error(`Error al consultar Marca ${tipo}: ${respuesta.status} - ${respuesta.statusText}`);
        ...
        console.error('Error al consultar Marca:', error);
        return null;
      }
    };
    ...
    const cargarDatosTipo = async (idABuscar) => {
      ...
      try {
        const marcaData = await obtenerTipo(idABuscar);
        if (marcaData) {

          formData.value.id = marcaData.id;
          formData.value.tipo_marca = marcaData.tipo_marca;
          formData.value.descripcion = marcaData.descripcion;

        } else {
          console.warn('No se encontraron datos para Id Marca  :', idABuscar);
          limpiarFormulario();
        }
      } catch (error) {
        console.error('Error al cargar datos para Id Marca:', error);
      }
    };

    const crearRegistro = async () => {
      try {
        const apiUrl = `${import.meta.env.VITE_API_URL}marca/`;
        console.log(apiUrl);
        ...
        if (response.ok) {
          ...
          mostrarMensaje('Marca creado exitosamente!', 'success');
          formData.value = { id: '', tipo_marca: '', descripcion: '' };
          cargarListaDeMarcas(); // Recargar la lista después de crear
        } else {
          const errorData = await response.json();
          mostrarMensaje(`Error al crear Marca: ${errorData.message || response.statusText}`, 'error');
          console.error('Error al crear Marca:', errorData);
        }
      } catch (error) {
    ...
    const modificarRegistro = async (id) => {
      try {
        ...
        if (response.ok) {
          ...
          const errorData = await response.json();
          mostrarMensaje(`Error al modificar Marca ${errorData.message || response.statusText}`, 'error');
          console.error('Error al modificar Marca:', errorData);
        }
      } catch (error) {
        mostrarMensaje('Error de conexión con el servidor.', 'error');
        console.error('Error de conexión:', error);
    ...
    const eliminarRegistroConfirmado = async () => {
      ...
      try {
        const apiUrl = `${import.meta.env.VITE_API_URL}marca/${registroAEliminarId.value}/`;
        ...
        if (response.ok) {
          mostrarMensaje('Marca Eliminado exitosamente!', 'success');
          cargarListaDeMarcas(); // Recargar la lista después de eliminar
        } else {
          const errorData = await response.json();
          mostrarMensaje(`Error al eliminar Marca: ${errorData.message || response.statusText}`, 'error');
          console.error('Error al eliminar Marca:', errorData);
        }
      } catch (error) {
        mostrarMensaje('Error de conexión con el servidor.', 'error');
        console.error('Error de conexión:', error);
    ...
    const cargarListaDeMarcas = async () => {
      try {
        const apiUrl = `${import.meta.env.VITE_API_URL}marca/`; // Endpoint para obtener la lista completa
        const response = await fetch(apiUrl);
        if (response.ok) {
          const data = await response.json();
          datos.value = data.data; // Asume que tu API devuelve la lista en data.data
        } else {
          console.error('Error al cargar la lista de Marcas:', response.statusText);
          mostrarMensaje('Error al cargar la lista de Marcas.', 'error');
        }
      } catch (error) {
        console.error('Error de conexión al cargar Marcas:', error);
        mostrarMensaje('Error de conexión al cargar Marcas.', 'error');
      }
    };
      ...
      <div class="card-body mt-3">
        ...
        <form @submit.prevent="handleSubmit">
          ...
          <div class="mb-3 row align-items-center">
            <label for="id" class="col-md-2 col-form-label negrita">Marcas</label>
            <div class="col-md-3">
          ...
          <div class="card">
            ...
            <div class="card-body left">
              ...
              <div class="mb-3">
                <label for="tipo_marca" class="form-label negrita">Código Tipo Marca</label>
                <input type="text" :size="5" class="form-control form-control-sm" id="tipo_marca"
                  v-model="formData.tipo_marca" required />
              </div>
        ...
        <h3 class="mt-4" style="font-weight: bolder; font-size: medium; color: rgb(56, 149, 73);">Lista de Marcas De
          Vehículos
        </h3>
        <table class="table table-striped">
          ...
          <tbody>
            ...
            <tr v-if="datos.length === 0">
              <td colspan="6">No hay Marcas de Vehículos Registrados.</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ¿Estás seguro de que deseas eliminar la Marca con ID: {{ registroAEliminarId }}?
          </div>
